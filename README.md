...
管理者がトレーラー(コンテナ)が輸送計画を立て、積卸作業員やトラックドライバーがそれらを参照して管理することが出来るシステム
ドライバーの予約システムに併用して、より運輸業に特化したシステムを考える

### 基本業務

#### 管理者側

- **在庫管理** : 自社倉庫に存在している在庫情報を管理
- **商品管理** : 自社が取り扱っている商品情報を管理
- **入庫管理** : どの商品がいつに入庫するかを管理
- **出庫管理** : どの在庫がいつに出庫するかを管理
- **輸送計画** : 入出庫において、どのドライバーがどのトラクターを使用してどのトレーラーにどの在庫を輸送するかを計画(輸送経路なども計画)
- **受注管理** : 受注している取引を確認
- **発注管理** : 発注している取引を確認
- **受注依頼管理** : 発注を依頼された取引(受注依頼)を管理する
- **従業員管理** : 自社の従業員の人事的な管理を行う
...
- **輸送計画確認** : 輸送計画を基に作業を確認する
- **輸送先連絡** : 輸送を開始した段階で発送先に予め連絡を行い、予約リストに追加できるようにする(輸送計画段階で予め決定しておくことも想定)
